...
public abstract class RuBaseBuilder extends BaseInterrogationDocBuilder {
    ...
    protected void addInlineSignatureLineWithLawyer(XWPFDocument doc, String roleLabel, String fio,
                                                    String lawyer) {
        addInlineSignatureLine(doc, roleLabel, fio);

        if (lawyer != null && !lawyer.isBlank()) {
            addInlineSignatureLine(doc, "Защитник (адвокат)", lawyer);
        }
    }
    protected void addClosingSection(XWPFDocument doc, CaseInterrogationFullResponse data,
        ...
        zaLabel.setText("Заявлений, замечаний, дополнений, уточнений и исправлений, подлежащих внесению в протокол ");
        zaLabel.setFontFamily(FONT);
        zaLabel.setFontSize(FONT_SIZE);
        XWPFRun zaValue = zaPara.createRun();
        zaValue.setUnderline(UnderlinePatterns.SINGLE);
        zaValue.setText(safe(data.getAdditionalInfo()).equals("—") ? "не имею" : data.getAdditionalInfo());
        zaValue.setFontFamily(FONT);
        zaValue.setFontSize(FONT_SIZE);
        addEmptyLine(doc);
        ...
        appValue.setText(safe(data.getApplication()).equals("—") ? "не прилагается" : data.getApplication());
        ...
        addInlineSignatureLineWithLawyer(doc, roleLabel, fio, data.getLawyer());
    ...
    public static final String RIGHTS_ST78 =
            "Свидетель имеет право:\n"
                    ...
                    + "6) приносить жалобы на действия дознавателя, следователя, прокурора и суда, заявлять ходатайства, в том числе о принятии мер безопасности.\n"
                    ...
                    + "За дачу ложных показаний, отказ от дачи показаний свидетель несет уголовную ответственность, предусмотренную Уголовным кодексом Республики Казахстан."
                    + "За уклонение от дачи показаний или неявку без уважительных причин по вызову органа, ведущего уголовный процесс, на свидетеля может быть наложено денежное взыскание в порядке, установленном статьей 160 настоящего Кодекса.";
