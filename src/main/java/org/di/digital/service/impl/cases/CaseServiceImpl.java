...
public class CaseServiceImpl implements CaseService {
        ...
        public CaseResponse editCase(Long caseId, EditCaseRequest request, String email) {
                CaseWriter.EditPrecheck pre = caseWriter.prepareEdit(caseId, email, request.getNumber());
                        ...
                        webClientBuilder.build()
                                        ...
                                        .onStatus(
                                                        ...
                                                        response -> Mono.error(new IllegalStateException(
                                                                        MessageConstant.WORKSPACE_ALREADY_EXISTS
                                                                                        .format(newNumber))))
                ...
                Map<Integer, List<CaseFile>> grouped = caseEntity.getFiles().stream()
                                .sorted(Comparator
                                                ...
                                                .thenComparing(CaseFile::getOrderIndex,
                                                                Comparator.nullsLast(Integer::compareTo))
                                                .thenComparing(CaseFile::getUploadedAt,
        ...
        public void updateCaseStatus(Long caseId, boolean status, String email, CaseRejectionReason reason) {
                log.info("Updating status for case: {} to {} by user: {} with reason: {}", caseId, status, email,
                                reason);
        ...
        public List<CaseFileResponse> addFilesToCase(Long caseId, List<MultipartFile> files,
                ...
                List<CaseFileWriter.UploadedFile> uploaded = uploadAndCount(files, ctx.caseNumber(),
                                ctx.existingNames());
        ...
        public void deleteFileFromCase(Long caseId, String fileName, String email) {
                ...
                try {
                        ...
                        logService.log(String.format("Deleted %s file from case %s", f.originalFileName(),
                                        f.caseNumber()),
        ...
        public InputStreamResource downloadFile(Long caseId, String originalFileName, String email) {
                ...
                logService.log(
                                String.format("Downloaded %s file from case %s", caseFile.getOriginalFileName(),
                                                caseNumber),
                                LogLevel.INFO,
        ...
        @Override
        @Transactional
        public FigurantResponse addFigurantToCase(Long caseId, AddFigurantToCaseRequest request,
                ...
                CaseFigurant saved = savedCase.getFigurants().stream()
                                .filter(f -> f.getFio().equals(request.getFio())
                                                && f.getNumber().equals(request.getNumber()))
                                .findFirst()
        ...
        @Transactional(readOnly = true)
        public Optional<FigurantResponse> findFigurantByNumber(Long caseId, String documentType, String number,
                        String email) {
                Case caseEntity = caseRepository.findById(caseId)
                                .orElseThrow(() -> new RuntimeException("Case not found"));
        ...
        private List<CaseFileWriter.UploadedFile> uploadAndCount(
                ...
                for (MultipartFile file : files) {
                        ...
                        try {
                                ...
                                Integer pages = null;
                                try {
                                        pages = pageCounter.countPagesByUrl(caseFile.getFileUrl(),
                                                        caseFile.getContentType());
                                } catch (Exception e) {
                                        log.warn("Could not count pages for {}: {}", caseFile.getOriginalFileName(),
                                                        e.getMessage());
                                }

                                uploaded.add(new CaseFileWriter.UploadedFile(
                                                caseFile.getOriginalFileName(), caseFile.getStoredFileName(),
                                                caseFile.getFileUrl(),
                                                caseFile.getContentType(), caseFile.getFileSize(),
                                                caseFile.getUploadedAt(), pages));
